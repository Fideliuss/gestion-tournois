...
//  N pièces de valeur X€ → conversion en euros
// ══════════════════════════════════════════════════════

const CV_QUESTIONS = 10;
const CV_DEFAULT_TIMERS = { facile: 12, medium: 8, expert: 4 };
const CHIP_VALUES = [2.5, 5, 10, 20, 50];

let _cvConfig    = { levels: CV_DEFAULT_TIMERS };
...
async function initConversion() {
  try {
    ...
    if (cfg && cfg.conversion) {
      _cvConfig = cfg;
      const lvl = cfg.conversion.levels || CV_DEFAULT_TIMERS;
...
async function startConversionLevel(level) {
  _cvLevel   = level;
  _cvTimer   = ((_cvConfig.conversion && _cvConfig.conversion.levels) || CV_DEFAULT_TIMERS)[level];
  _cvQIndex  = 0;
  _cvCorrect = 0;
  _cvAnswered = false;
...
function nextConversion() {
  ...
  _cvValue = CHIP_VALUES[Math.floor(Math.random() * CHIP_VALUES.length)];

  document.getElementById('cv-chips').textContent       = _cvChips;
  document.getElementById('cv-value-badge').textContent = '× ' + formatVal(_cvValue) + ' €';
...
function cvTimeout() {
  ...
  const correct = roundVal(_cvChips * _cvValue);
  ...
  try {
    if (_cvSessionId && _cvUserId) {
      await SB.addTrainingResult(
        ...
        { chips: _cvChips, value: _cvValue, level: _cvLevel },
...
async function showConversionSummary() {
  ...
  document.getElementById('cv-summary-level').textContent   = lbls[_cvLevel] + ' · ' + _cvTimer + 's';
